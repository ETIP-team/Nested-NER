...
model_dictionary = "../../model/ace05/cancel_nested_cnn_nested_classifier/" + str(cfg.NESTED_CLASSES_NUM) + \
                   "_cls_kernel_size_" + str(
...
def prepare_test_data():
    ...
    path = "../../dataset/ace05/extent/test.data"
    ...
    for sub_ls in ls_data:
        ...
        whole_sentence_ndarray = str2wv(sub_ls[0].split(), word_vector_model)
        ...
        for index in range(len(gt_bboxs)):
            ...
            # one_train_data = whole_sentence_ndarray[one_bbox[0]: one_bbox[1]].tolist()
            one_train_data = padding_entity(whole_sentence_ndarray[one_bbox[0]: one_bbox[1], :])
...
def load_model(model_path):  # , word_dic):
...
def main():
    ...
    for epoch_index in range(start_test_epoch + 1, max_epoch_time):
        ...
        # test_data, test_labels = prepare_test_data(word_dic)
