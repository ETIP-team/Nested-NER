...
entity_distribution = [0] * (len(cfg.LABEL) - 1)
...
model_save_dictionary = "../../model/genia/cancel_nested_cnn_nested_classifier/" + str(cfg.NESTED_CLASSES_NUM) + \
                        "_cls_kernel_size_" + str(
    cfg.KERNEL_LENGTH) + "_len_" + str(cfg.MAX_CLS_LEN) + "_class_weight_" + "-".join(
    [str(i) for i in cfg.CLASS_WEIGHT]) + "/"
...
def prepare_train_data():  # todo need modity the sentence to idx.
        ...
        for index in range(len(gt_bboxs)):
            ...
            if cfg.NESTED_CLASSES_NUM == 4:
                nested_label = make_nested_label_nested_cancel(np.array(one_bbox), label, gt_classes,
                                                               np.array(gt_bboxs))
            # nested_label = make_nested_label_nested_cancel(np.array(one_bbox), label, gt_classes, np.array(gt_bboxs))
