...
class BaseArguments:
    def __init__(self, normalize, fold_k, dx_compute_method, pos_loss_method, prevent_overfitting_method, th_train_iou,
                 partial_l2_penalty=True, loss_weight_lambda=1.0):
    ...
    def show_arguments(self):
        ...
        elif isinstance(self, TestAruguments):
    ...
    def get_model_path(self, model_epoch):
        path = f"model/genia/max_cls_len{cfg.MAX_CLS_LEN}_kernal_size_" + str(
            cfg.KERNAL_LENGTH) + "_pooling_out_" + str(
            cfg.POOLING_OUT)  #
        ...
        if isinstance(self, TestAruguments):
            print("Load Model from")
        elif isinstance(self, TrainArguments):
            print("Save Model in")
        print(path)
        return path
...
class TrainArguments(BaseArguments):
    def __init__(self, start_save_epoch, pos_loss_method, normalize, th_train_iou, max_iter_epoch,
        ...
        super(TrainArguments, self).__init__(normalize, fold_k, dx_compute_method, pos_loss_method,
                                             prevent_overfitting_method, th_train_iou, partial_l2_penalty,
                                             loss_weight_lambda)
        self.is_train = True
    ...
    def get_save_directory(self):
        path = f"model/genia/max_cls_len{cfg.MAX_CLS_LEN}_kernal_size_" + str(
            cfg.KERNAL_LENGTH) + "_pooling_out_" + str(cfg.POOLING_OUT)
        path += "norm_" + self.dx_compute_method + "_" if self.normalize else ""
        path += self.pos_loss_method + "_"
        if self.prevent_overfitting_method.lower() == "l2 regu":
            path += "partial_l2_" if self.partial_l2_penalty else "all_l2_"
    ...
    def get_train_data_path(self):
        ...
        # return "dataset/debug/train_debug_train_th_iou_" + str(self.th_train_iou) + ".npz"
...
class TestAruguments(BaseArguments):
    def __init__(self, normalize, pos_loss_method, th_train_iou, min_test_epoch, max_test_epoch,
                 ...
                 , fold_k=cfg.K_FOLD, with_regressor=True):
        super(TestAruguments, self).__init__(normalize, fold_k, dx_compute_method, pos_loss_method,
                                             prevent_overfitting_method, th_train_iou, partial_l2_penalty,
                                             loss_weight_lambda)
        self.is_train = False
        self.cuda = cuda
    ...
    def get_test_npz_path(self):
        ...
        path = f'dataset/test/test_with_BI_crf_max_cls_len{cfg.MAX_CLS_LEN}.npz'

        return path

    def get_write_result_path(self):
        path = f"./result/genia/with_BI_crf_max_cls_len{cfg.MAX_CLS_LEN}_"
...
#     output_flow = False
#     th_score = 0.8
#     testarguments = TestAruguments(norm, pos_loss_method, th_train_iou, min_test_epoch, max_test_epoch,
