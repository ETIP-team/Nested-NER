...
def make_contain_label(region_proposal, region_proposal_cls, gt_classes, gt_boxes):
...
def process_data_train(pkl_file_path, save_path, th_iou_train):
    ...
    for sample_index in range(sample_num):
        ...
        for roi_index in range(nroi):
            ...
            if max_ious[roi_index] >= th_iou_train:
                ...
                pos_idx.append(gid)
                contain_label = make_contain_label(bboxs[roi_index].copy(), gt_classes[max_idx[roi_index]],
                                                   gt_classes.copy(), gt_boxes.copy())
                train_contain_label.append(contain_label)
                # contain_label = append()
                ...
                # cover_nega.write("")
                gid = len(train_roi)
                train_contain_label.append(-1)  # fill empty value
                train_roi.append(rbbox[roi_index])
                train_tbbox.append(tbbox[roi_index])
                neg_idx.append(gid)
                train_cls.append(0)
    ...
    train_contain_label = np.array(train_contain_label)
    ...
    np.savez(open(save_path, 'wb'), train_sentences=train_sentences, train_contain_label=train_contain_label,
             train_sentence_info=train_sentence_info, train_roi=train_roi,
             train_cls=train_cls, train_tbbox=train_tbbox, train_norm_tbbox=train_norm_tbbox)
...
def process_data_train_k_fold():
    ...
    train_pkl_file_path = f'dataset/train/train_base_max_cls_len{cfg.MAX_CLS_LEN}.pkl'
    save_folder = f'dataset/train/train_base_with_contain_max_cls_len{cfg.MAX_CLS_LEN}_'
...
def process_data_test(pkl_file_path, save_path):
    ...
    for sample_index in range(sample_num):
        ...
        for index in range(len(gt_boxes)):
            one_contain_label = make_contain_label(gt_boxes[index], gt_classes[index], gt_classes, gt_boxes)
            contain_labels.append(one_contain_label)
...
def process_data_test_k_fold():
    # test_pkl_file_path = f'dataset/test/test_with_BI_crf_max_cls_len{cfg.MAX_CLS_LEN}.pkl'
    # save_folder = f'dataset/test/test_with_BI_crf_max_cls_len{cfg.MAX_CLS_LEN}'
    test_pkl_file_path = f'dataset/test/test_base_max_cls_len{cfg.MAX_CLS_LEN}.pkl'
    save_folder = f'dataset/test/test_base_with_contain_max_cls_len{cfg.MAX_CLS_LEN}'
    # test_pkl_file_path = 'dataset/debug/debug.pkl'
    # save_folder = 'dataset/debug/debug_test'
