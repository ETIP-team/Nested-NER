...
import numpy as np
from _4_20_bilstm_attention_one_layer.attention_neww2vmodel import geniaDataset

from _4_20_bilstm_attention_one_layer.one_layer_config import Config
...
def data_prepare_one_layer(config: Config, path: str, word_dict: geniaDataset) -> (list, list, list):
    # read file path and ...
    #
    """    :param config: Config file
    :param path: data file path
    ...
    for data_index in range(len(data_list)):
        ...
        temp = data_list[data_index].split("\n")
        assert config.max_nested_level == 1
        max_nested_level = config.max_nested_level
        one_seq_labels = [[] for i in range(max_nested_level)]
        ...
        word_ids = word2ids(words, word_dict)
        nested_index = config.nested_level
        one_layer_labels = [config.bio_labels.index(sentence_info[i][nested_index]) for i in
                            range(len(sentence_info))]

        one_seq_labels[0].extend(one_layer_labels)

        empty_label = [0 for i in range(len(sentence_info))]
        if one_seq_labels[0] == empty_label:
            continue
